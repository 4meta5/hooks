...
describe('add command', () => {
  ...
  describe('--cwd option', () => {
    // Helper to create a local skill in a source directory
    async function createLocalSkill(sourceDir: string, skillName: string): Promise<void> {
      await mkdir(join(sourceDir, '.claude', 'skills', skillName), { recursive: true });
      await writeFile(
        join(sourceDir, '.claude', 'skills', skillName, 'SKILL.md'),
        `---
name: ${skillName}
...
`,
    ...
    it('should install skills to specified directory instead of cwd', async () => {
      ...
      // Create source project with a skill
      const sourceDir = await mkdtemp(join(tmpdir(), 'skills-source-'));
      try {
        await createLocalSkill(sourceDir, 'test-cwd-skill');
    ...
    it('should track project installation when adding skill', async () => {
      ...
      const sourceDir = await mkdtemp(join(tmpdir(), 'skills-source-'));
      try {
        await createLocalSkill(sourceDir, 'track-skill');
    ...
    it('should track multiple skills in same project', async () => {
      ...
      // Create source project with two skills
      const sourceDir = await mkdtemp(join(tmpdir(), 'skills-source-'));
      try {
        await createLocalSkill(sourceDir, 'multi-skill-a');
        await createLocalSkill(sourceDir, 'multi-skill-b');
        ...
        try {
          // Add two skills
          await addCommand(['multi-skill-a'], { cwd: targetDir });
          await addCommand(['multi-skill-b'], { cwd: targetDir });

          const installation = await getProjectInstallation(targetDir);

          expect(installation?.skills).toContain('multi-skill-a');
          expect(installation?.skills).toContain('multi-skill-b');
        } finally {
          process.chdir(originalCwd);
        }
      } finally {
    ...
    it('should not duplicate skill references when adding same skill twice', async () => {
      ...
      const sourceDir = await mkdtemp(join(tmpdir(), 'skills-source-'));
      try {
        await createLocalSkill(sourceDir, 'dupe-skill');
        ...
        try {
          ...
          await addCommand(['dupe-skill'], { cwd: targetDir });
          await addCommand(['dupe-skill'], { cwd: targetDir });

          // Read CLAUDE.md and count references
          const content = await readFile(join(targetDir, 'CLAUDE.md'), 'utf-8');
          const matches = content.match(/@\.claude\/skills\/dupe-skill\/SKILL\.md/g);
